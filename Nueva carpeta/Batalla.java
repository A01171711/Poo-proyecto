...
public class Batalla{
	...
	public static void iniciarBatalla(PerrosSuper p1, PerrosMalos p2){
		while(p1.getHp()>0&&p2.getHp()>0){
			...
			switch(sc.nextInt()){
				...
				case 3:
					...
					for(int i=0;i<p1.getCorrea().length;i++){
						if(p1.getCorrea()[i]!=null){
							System.out.print((i+1)+" "+p1.getCorrea()[i].getNombre()+" ["+p1.getCorrea()[i].getPuntos()+"] ");
						}else{
					...
					if(p1.getCorrea()[opcionElegida-1].getClass()==Arma.class)
						p1.atacar(p2,p1.getCorrea()[opcionElegida-1]);
					else
